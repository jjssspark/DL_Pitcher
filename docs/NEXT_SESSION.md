...
**비용이 특징마다 다르다. 캐시는 `(frame_idx, [cx, cy, conf])`만 담는다 — 박스 크기는
저장하지 않았고 mp4도 처리 후 삭제했다.**

- 좌표만으로 되는 것 (재수집 불필요, 즉시 검증 가능): 릴리스 포인트, 구간별 곡률 비,
  가속도, 궤적 방향 변화 등 기하 특징 전부
- 재다운로드 + 재감지가 필요한 것 (4경기 약 100분): 박스 크기 변화율

먼저 좌표 기반 특징만으로 OFFSPEED f1이 움직이는지 본 뒤, 필요하면 박스 크기를 위해
재수집한다. 그때는 `extract_trajectory_candidates`가 박스 크기도 캐시에 남기도록
고쳐야 두 번 받지 않는다.

단점: 특징 추가가 성능으로 이어진다는 보장이 없다.
