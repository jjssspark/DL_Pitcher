...
with st.sidebar:
    ...
    if st.session_state._local_video_path and st.session_state.video_src:
        ...
        st.markdown(
            ...
            f'싱크: {_sa_txt}<br>'
            f'감지된 투구: {_ocr_cnt}구 | 현재: #{c_idx+1 if _sa else "—"} / {len(pitches)}'
...
with col_video:
    ...
    if _yt:
        ...
        _yt_result = _yt_player(
            video_id=_yt, start_sec=0,
            seek_to=_seek_to, is_playing=_is_playing,
            key="yt_main",
        )
        ...
        _local_path = st.session_state.get("_local_video_path")
