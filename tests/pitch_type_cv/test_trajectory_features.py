...
def test_longest_moving_chain_trims_static_tail():
    ...
    candidates = [
        ...
        (3, [(676.0, 262.0, 0.6)]),
        (4, [(672.0, 255.0, 0.6)]),
        (5, [(670.0, 252.0, 0.6)]),   # 이후 정지
        (6, [(670.0, 252.0, 0.6)]),
        (7, [(670.0, 252.0, 0.6)]),
    ]
    chain = longest_moving_chain(candidates, max_jump_px=60.0, min_total_move_px=30.0)

    assert (670.0, 252.0) not in chain
    assert chain == [
        (700.0, 300.0), (690.0, 285.0), (682.0, 272.0), (676.0, 262.0), (672.0, 255.0),
    ]


def test_longest_moving_chain_rejects_static_segment_in_the_middle():
    from pitch_type_cv.trajectory_features import longest_moving_chain

    # 실측 재현(TS-019, 투구 #53): 공 3프레임 → 정지 오탐 5프레임 → 공 5프레임.
    # gap 2 관용 때문에 공에서 정지 물체로 건너뛰어 길이를 벌 수 있다.
    # 앞뒤 트림으로는 못 잡는다 — 정지 구간이 사슬 한가운데 있기 때문이다.
    candidates = [
        (44, [(720.0, 357.0, 0.50)]),
        (46, [(742.0, 292.0, 0.45)]),
        (47, [(738.0, 276.0, 0.21)]),
        (48, [(721.0, 356.0, 0.18)]),   # 여기부터 정지 오탐
        (49, [(721.0, 356.0, 0.25)]),
        (50, [(721.0, 356.0, 0.22)]),
        (51, [(721.0, 356.0, 0.22)]),
        (52, [(722.0, 356.0, 0.28)]),
        (54, [(706.0, 246.0, 0.57)]),   # 다시 진짜 공
        (55, [(700.0, 254.0, 0.69)]),
        (56, [(694.0, 263.0, 0.73)]),
        (57, [(687.0, 276.0, 0.33)]),
        (59, [(674.0, 306.0, 0.69)]),
    ...
    assert (721.0, 356.0) not in chain
    assert (722.0, 356.0) not in chain
    assert chain == [
        (706.0, 246.0), (700.0, 254.0), (694.0, 263.0), (687.0, 276.0), (674.0, 306.0),
    ]
